    ...
    public void pre(File configFolder) {
        ...
        if (!target.exists()) {
            ...
            for (String s : this.files) {
                INpureCore.proxy.print("Extracting: " + s + " to " + target.getAbsolutePath()
                        + System.getProperty("file.separator") + s + ".");
                Reader r = Streams.instance.getReader(
                        getClass().getClassLoader().getResourceAsStream("scripts/custom_nei_filters/" + s));
