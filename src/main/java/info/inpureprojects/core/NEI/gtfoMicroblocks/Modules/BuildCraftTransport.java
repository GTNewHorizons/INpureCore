...
public class BuildCraftTransport
    ...
    public void run() {
        ...
        try {
            LinkedList<ItemStack> facades = (LinkedList<ItemStack>) Class.forName("buildcraft.transport.ItemFacade").getDeclaredField("allFacades").get(null);
            API.setItemListEntries(facades.get(0).getItem(), Arrays.asList(facades.get((new Random()).nextInt(facades.size()))));
        } catch (Throwable t) {
